<template>
    <modal name="assignOrderMessageModal" height="auto" width="95%" :scrollable="true" classes="modal" :clickToClose="false" @before-open="beforeOpen" @opened="opened">
        <h4 class="modal-title font-bold text-xl">{{ $t('Assign message') }}</h4>
        ...
        <form method="post" :action="route('order.message_assign')">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-default" @click="$modal.hide('assignOrderMessageModal')">{{ $t('Cancel') }}</button>
                <button type="submit" class="btn btn-primary" id="save-assign-order-message">{{ $t('Save') }}</button>
            </div>
        </form>
    </modal>
