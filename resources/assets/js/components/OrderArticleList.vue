    ...
    <div>
        <div class="panel panel-primary" v-for="(article, index) in articles" :key="index">
            <div class="panel-body row">
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <div class="form-control-static">
                            {{ article.name }}
                            <button type="button" class="btn btn-primary btn-xs" v-if="supplier" v-bind:class="{ 'm-l-md': (article.id) }" data-toggle="modal" data-target="#articleSelectModal" @click.prevent="showArticleList(index)">Select Article</button>
                            <div class="text-danger" v-if="!supplier">Please select a supplier first!</div>
                        </div>
                    </div>
                ...
                <div class="col-lg-2 text-right">
                    <div class="form-group">
                        <label for="price[]" class="control-label">Net Price per Unit</label>
                        <input class="form-control text-right price-select" required="required" name="price[]" id="price[]" type="text" v-model="article.price">
                    </div>
                </div>
                <div class="col-lg-2 text-right">
                    <div class="form-group">
                        <label :for="'expected_delivery' + index" class="control-label">Delivery Date</label>
        ...
        <button class="btn btn-primary btn-sm" @click.prevent="addArticle(true)">Add Article</button>
    </div>
</template>
